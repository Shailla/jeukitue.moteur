...
void CGeoObject::setSubMat( int *tab )
{
	...
	m_TabSubMat = new int[ m_NumFaces ];			// Création du tableau des références sous-matériaux	
...
void CGeoObject::AfficheWithMaterialTexture(CMaterialTexture *mat)
{
	cout << endl << "Texture : " << this->m_Nom;
		// Initialisation pour la texture
	float *texVertex = TabTex[ mat->m_CanalTex ]->TexVertex;
	int *texFaces = TabTex[ mat->m_CanalTex ]->TexFaces;
...
{
	cout << endl << "Multi : " << this->m_Nom;

	for( int i=0; i < m_NumTexFaces ; i++ )	//dessine toutes les faces de la map
	{
		if( mat->m_TabMat[ m_TabSubMat[i] ]->Type() == CMaterial::MAT_TYPE_TEXTURE )
		{
			CMaterialTexture *matTex = (CMaterialTexture*)mat->m_TabMat[ m_TabSubMat[i] ];
			float *texVertex = TabTex[ matTex->m_CanalTex ]->TexVertex;
			int *texFaces = TabTex[ matTex->m_CanalTex ]->TexFaces;
...
void CGeoObject::Affiche()
{
	...
	if( m_bMaterialTexture && m_Material )	// Y a-t-il un matériau associé
	{
		...
		if( m->Type() == CMaterial::MAT_TYPE_SIMPLE )			// Un matériau minimaliste est associé (propriétés diffuses, ambiantes et spéculaires)
			...
			AfficheWithMaterialTexture( (CMaterialTexture*)m );
...
{
	int nbrMat = matRef + m_OffsetMateriaux;	// Décalage de la référence matériau de l'offset demandé
	...
	{
		cerr << endl << "Erreur (CGeoObject::setMaterial) : Materiau introuvable1 " << nbrMat << endl;
	}
	else 
	{
...
bool CGeoObject::LitFichierGeoObject( CIfstreamMap &fichier )
{
	...
	for( int k=0 ; k<nbrCanaux ; k++ )
	{
		...
		TabTex[ canal ]->TexVertex = new float[3*m_NumTexVertex];
		float *texVertex = TabTex[ canal ]->TexVertex;
		...
		TabTex[ canal ]->TexFaces = new int[ 3*m_NumTexFaces ];
		int *texFaces = TabTex[ canal ]->TexFaces;
...
{	
	...
	fichier << "\tNom\t\t\t\t\"" << m_Nom << '\"';	// Sauvegarde du nom de l'objet
	...
	for( i=0 ; i<m_NumFaces ; i++ )
	{
		...
		{
			cout << endl << "Geo avec matMulti : " << m_Nom << endl;
	...
	for( map<int,CChanTex*>::iterator p=TabTex.begin() ; p!=TabTex.end() ; p++ )
	{
		...
		float *texVertex = (*p).second->TexVertex;
		int *texFaces = (*p).second->TexFaces;
...
void CGeoObject::OffsetMateriau( int offset )
{
