...
extern DECLSPEC AG_DataSource *AG_OpenCore(void *, size_t);
extern DECLSPEC AG_DataSource *AG_OpenConstCore(const void *, size_t);
...
extern DECLSPEC AG_IOStatus AG_Read(AG_DataSource *, void *, size_t, size_t);
extern DECLSPEC AG_IOStatus AG_ReadAt(AG_DataSource *, void *, size_t, size_t, off_t);
extern DECLSPEC AG_IOStatus AG_Write(AG_DataSource *, const void *, size_t, size_t);
extern DECLSPEC AG_IOStatus AG_WriteAt(AG_DataSource *, const void *, size_t, size_t, off_t);
extern DECLSPEC off_t AG_Tell(AG_DataSource *);
extern DECLSPEC int AG_Seek(AG_DataSource *, off_t, enum ag_seek_mode);
...
extern DECLSPEC int AG_WriteTypev(AG_DataSource *, Uint32);
extern DECLSPEC int AG_CheckTypev(AG_DataSource *, Uint32);
#define AG_LockDataSource(ds) AG_MutexLock(&(ds)->lock);
#define AG_UnlockDataSource(ds) AG_MutexUnlock(&(ds)->lock);
#define AG_WriteType(ds,code) do { if ((ds)->debug) AG_WriteTypeCode((ds),(code)); } while (0)
#define AG_CHECK_TYPE(ds,code,rv) do { if ((ds)->debug && AG_CheckTypev(ds,code) == -1) { return (rv); } } while (0)
#define AG_WriteTypeAt(ds,code,off) do { if ((ds)->debug) AG_WriteTypeCodeAt((ds),(code),(off)); } while (0)
