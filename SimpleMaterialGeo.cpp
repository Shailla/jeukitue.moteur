...
CSimpleMaterialGeo::CSimpleMaterialGeo(CMap* map)
	:CGeo( map )
{
	m_NumVertex = 0;
	m_TabVertex = NULL;			// Pointeur sur le tableau de sommets
...
bool CSimpleMaterialGeo::Lit(TiXmlElement* element)
{
	double ref;
...
void CSimpleMaterialGeo::GereContactPlayer( const float pos[3], CPlayer *player )
{
...
float CSimpleMaterialGeo::GereLaser( float pos[3], CV3D &Dir, float dist)
{
	float distanceVar;
	float *vertex, *normal;
