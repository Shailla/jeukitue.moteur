...
bool CTextureMaterialGeo::Lit(TiXmlElement* element)
{
	...
	if(!nom)
		throw CErreur(0, "Fichier Map corrompu CTextureMaterialGeo 1");
	setName(nom);
	...
	if(!element->Attribute(Xml::REF, &ref))
		throw CErreur(0, "Fichier Map corrompu CTextureMaterialGeo 4");
...
void CTextureMaterialGeo::GereContactPlayer( const float pos[3], CPlayer *player )
{	
...
float CTextureMaterialGeo::GereLaser( float pos[3], CV3D &Dir, float dist)
{
	float distanceVar;
	float *vertex, *normal;
