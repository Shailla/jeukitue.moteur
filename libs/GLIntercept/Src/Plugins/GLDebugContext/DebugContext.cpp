...
static StrLookup s_debugSourceLookup[] =
{
  ...
  StrLookup(GL_DEBUG_SOURCE_API_ARB,             "OpenGL"),
  StrLookup(GL_DEBUG_SOURCE_WINDOW_SYSTEM_ARB,   "Windows"),
  StrLookup(GL_DEBUG_SOURCE_SHADER_COMPILER_ARB, "Shader Compiler"),
  StrLookup(GL_DEBUG_SOURCE_THIRD_PARTY_ARB,     "Third Party"),
  StrLookup(GL_DEBUG_SOURCE_APPLICATION_ARB,     "Application"),
  StrLookup(GL_DEBUG_SOURCE_OTHER_ARB,           "Other"),
};
...
static StrLookup s_debugTypeLookup[] =
{
  ...
  StrLookup(GL_DEBUG_TYPE_ERROR_ARB,               "Error"),
  StrLookup(GL_DEBUG_TYPE_DEPRECATED_BEHAVIOR_ARB, "Deprecated behavior"),
  StrLookup(GL_DEBUG_TYPE_UNDEFINED_BEHAVIOR_ARB,  "Undefined behavior"),
  StrLookup(GL_DEBUG_TYPE_PORTABILITY_ARB,         "Portability"),
  StrLookup(GL_DEBUG_TYPE_PERFORMANCE_ARB,         "Performance"),
  StrLookup(GL_DEBUG_TYPE_OTHER_ARB,               "Message"),
};
...
static StrLookup s_debugSeverityLookup[] =
{
  ...
  StrLookup(GL_DEBUG_SEVERITY_HIGH_ARB,    "High"),
  StrLookup(GL_DEBUG_SEVERITY_MEDIUM_ARB,  "Medium"),
  StrLookup(GL_DEBUG_SEVERITY_LOW_ARB,     "Low"),
};
...
///////////////////////////////////////////////////////////////////////////////
//
void GLAPIENTRY Custom_glDebugMessageControlARB(GLenum source, GLenum type, GLenum severity, GLsizei count, const GLuint * ids, GLboolean enabled)
{
  // If there is no context or there are rules, ingnore this call
  if(!g_contextData ||
     !g_contextData->iglDebugMessageControlARB ||
  ...
  g_contextData->iglDebugMessageControlARB(source, type, severity, count, ids, enabled);
}

///////////////////////////////////////////////////////////////////////////////
//
void GLAPIENTRY Custom_glDebugMessageCallbackARB(GLDEBUGPROCARB callback, GLvoid* userParam)
{
  if(!g_contextData ||
...
void GLAPIENTRY Custom_glGetPointerv(GLenum pname, void** params)
{
  ...
  if(pname == GL_DEBUG_CALLBACK_FUNCTION_ARB)
  {
    *params = g_contextData->m_currContext->m_appCallback;
  }
  else if(pname == GL_DEBUG_CALLBACK_USER_PARAM_ARB)
  {
...
{
  // If not the disable for synchronous output, allow the call
...
iglDebugMessageControlARB(NULL),
iglDebugMessageCallbackARB(NULL)
{
...
void DebugContext::OnGLContextSet(HGLRC oldRCHandle, HGLRC newRCHandle)
{
  ...
  if(iglDebugMessageControlARB == NULL ||
     iglDebugMessageCallbackARB == NULL)
  {
    if(gliCallBacks->IsGLExtensionSupported("GL_ARB_debug_output"))
    {
      iglDebugMessageControlARB = (void (GLAPIENTRY *)(GLenum source, GLenum type, GLenum severity, GLsizei count, const GLuint * ids, GLboolean enabled))gliCallBacks->GetWGLFunctions()->glGetProcAddress("glDebugMessageControlARB");
      iglDebugMessageCallbackARB = (void (GLAPIENTRY *)(GLDEBUGPROCARB callback, GLvoid* userParam))gliCallBacks->GetWGLFunctions()->glGetProcAddress("glDebugMessageCallbackARB");

      gliCallBacks->AddOverrideFunction("glDebugMessageControlARB", Custom_glDebugMessageControlARB, iglDebugMessageControlARB);
      gliCallBacks->AddOverrideFunction("glDebugMessageCallbackARB", Custom_glDebugMessageCallbackARB, iglDebugMessageCallbackARB);
      gliCallBacks->AddOverrideFunction("glGetPointerv", Custom_glGetPointerv, gliCallBacks->GetCoreGLFunctions()->glGetPointerv);

      // Override glDisable to catch GL_DEBUG_OUTPUT_SYNCHRONOUS_ARB
...
bool DebugContext::Context::Init(InterceptPluginCallbacks *gliCallBacks)
{
  ...
  m_contextDebugOutput = gliCallBacks->IsGLExtensionSupported("GL_ARB_debug_output");
  if(m_contextDebugOutput)
  {
    // Register the callback (still do this outside the m_synchronousDebug option?)
    if(g_contextData->iglDebugMessageCallbackARB)
    {
      g_contextData->iglDebugMessageCallbackARB(GLDebugOutput, NULL);
    }

    // Force sync mode - glDisable is intercepted to ensure the application does not change the setting.
    gliCallBacks->GetCoreGLFunctions()->glEnable(GL_DEBUG_OUTPUT_SYNCHRONOUS_ARB);

    // Set all the rules on the context
    if(g_contextData->iglDebugMessageControlARB)
    {
      for(uint i = 0; i < g_contextData->m_controlRules.size(); i++)
      {
        ...
        g_contextData->iglDebugMessageControlARB(rule.m_source, rule.m_type, rule.m_severity, (GLsizei)rule.m_ids.size(), idArray, rule.m_enabled);
      }
    }
  }
