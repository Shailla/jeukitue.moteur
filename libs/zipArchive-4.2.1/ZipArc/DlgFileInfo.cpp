...
BOOL CDlgFileInfo::OnInitDialog() 
{
	...
	if (m_pArchive || m_pHeader)
	{
		if (m_pHeader)
		{
			...
			else
			{
				...
				if (m_pHeader->IsEncrypted() && 
					m_pHeader->GetDataSize(true) == m_pHeader->m_uUncomprSize)
			...
			m_szInfo.Format(IDS_FILEINFO, szDir, szEncr, m_pHeader->m_uVolumeStart + 1, CZipArcApp::FormatSize(m_pHeader->m_uOffset), szC,
				...
				((float)(m_pHeader->m_uVersionMadeBy)/10), ((float)(m_pHeader->m_uVersionNeeded))/10);
		...
		else 
		{
			...
			if (!m_pArchive->GetStorage()->IsSegmented())
				szType.LoadString(IDS_ARINORMAL);
			else
			{
				szType.Format(IDS_ARIDS, m_pArchive->GetStorage()->IsSplit() ? _T("split") : _T("spanned"), CString((LPCTSTR)(m_pArchive->GetStorage()->IsExisting() ? IDS_ARIEX : IDS_ARICR )));
			}
			
			CWaitCursor cw;		
			ZIP_INDEX_TYPE iCount = m_pArchive->GetCount();
			
			// we need to calculate this
			ZIP_SIZE_TYPE uTotalDirSize = m_pArchive->GetCentralDirSize(false);
			ZIP_SIZE_TYPE uTotalUncompressed = 0, uTotalCompressed = 0;
			if (iCount)
			{
			...
			m_szInfo.Format(IDS_ARCHINFO, szType, CString((LPCTSTR)(m_pArchive->IsReadOnly() ? IDS_YES : IDS_NO)),
				CZipArcApp::FormatSize(m_pArchive->GetCount()),
				(m_pArchive->GetStorage()->IsNewSegmented() || !m_pArchive->GetStorage()->IsSegmented()) ? m_pArchive->GetCurrentVolume() : info.m_uLastVolume + 1, 
				CZipArcApp::FormatSize(uTotalUncompressed),
				ratio, 
				CZipArcApp::FormatSize(uTotalDirSize),
				CString((LPCTSTR)(info.m_bInArchive ? IDS_YES : IDS_NO)));
		}
		m_szInfo.Replace(_T("\n"), _T("\r\n"));
		m_bInfo = false;
	}
