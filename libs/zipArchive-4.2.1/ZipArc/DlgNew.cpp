...
void CDlgNew::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_BINARY, m_optionsNew.m_bBinarySplit);
...
{
	BOOL bEnableSplit = ((CButton*)GetDlgItem(IDC_TDSPAN))->GetCheck() && m_span.GetCheck();
 	GetDlgItem(IDC_VOL)->EnableWindow(bEnableSplit);
	GetDlgItem(IDC_BINARY)->EnableWindow(bEnableSplit);
	GetDlgItem(IDC_STATICVOL)->EnableWindow(bEnableSplit);
}
