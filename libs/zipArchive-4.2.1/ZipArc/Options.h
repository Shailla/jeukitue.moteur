...
struct COptionsCompress
{
	...
	int		m_iAlgorithm;	// compression algorithm: 0: deflate, 1: bzip2
	void Serialize(CArchive &ar)		
	{
		if (ar.IsStoring())	
		{
			ar << m_iLevel << m_iSystem << m_bRecursive << m_bReplace << m_iReplaceAction << m_iFileNames << m_iAlgorithm;
		}
		else
		{
			ar >> m_iLevel >> m_iSystem >> m_bRecursive >> m_bReplace >> m_iReplaceAction >> m_iFileNames >> m_iAlgorithm;
...
struct COptionsNew
{
	...
	BOOL	m_bBinarySplit;	// splitting is binary
	void Serialize(CArchive &ar)		
	{
		if (ar.IsStoring())	
		{
