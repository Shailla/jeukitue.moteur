...
* Copyright (c) 2000 - 2012 Artpol Software - Tadeusz Dracz
...
#ifdef _ZIP_TRIAL
,m_zip(_T("ZipArchive.license"))
#endif
{
	m_bAfterException = false;
	EnableAutomation();
	AfxOleLockApp();
	m_zip.SetSegmCallback(&m_changeCallback);

	m_zip.SetCallback(&m_progress, CZipActionCallback::cbAll);
...
ZIP_INDEX_TYPE CZipArcDoc::GetIndex(ZIP_INDEX_TYPE nListIndex)
{
...
void CZipArcDoc::Init(CZipArcView *pView)
{
	...
	MenuSaveFinalizeChange(IsSegmented(), ModifyMenu(false));
	
	SetSorted();
}

CZipFileHeader* CZipArcDoc::GetFileInfo(ZIP_INDEX_TYPE idx)
{
...
{
	pCmdUI->Enable((IsExisting() && IsSegmented() || !IsSegmented()) && m_zip.GetCount() > 0 && m_pView->GetListCtrl().GetSelectedCount() && !IsActionInProgress());
}

void CZipArcDoc::OnUpdateCommandExtract(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable((IsExisting() && IsSegmented() || !IsSegmented()) && m_zip.GetCount() > 0  && !IsActionInProgress());
}
...
{
	pCmdUI->Enable(!IsSegmented() && !m_zip.IsReadOnly() && m_pView->GetListCtrl().GetSelectedCount() && !IsActionInProgress());
}
...
{
	if (m_progress.m_iType == CZipActionCallback::cbSave && IsSegmented()
...
void CZipArcDoc::CProgressData::Begin(ZIP_SIZE_TYPE uMax)
{
	m_uPos = 0; 
	m_uMax = uMax;
	m_iType = CZipActionCallback::cbNothing;
	m_bInit = true;
	m_sStop.Deactivate();
	m_sAbort.Deactivate();
	//	Send();
...
void CZipArcDoc::CProgressData::SetTotal(ZIP_SIZE_TYPE uTotalToDo)
{
	CZipActionCallback::SetTotal(uTotalToDo);
	if (IsSubAction())
		m_bInit = true;
	else if (m_iType == CZipActionCallback::cbDelete) // we hide it here, because earlier we had only counting sub-action
...
void CZipArcDoc::CProgressData::Finish(bool bModifyMenu)
{
	...
	Send();
	if (m_iType == CZipActionCallback::cbAddTmp || m_iType == CZipActionCallback::cbAddStore)
		m_iType = CZipActionCallback::cbAdd;


	if (!m_iType || m_iType == CZipActionCallback::cbAdd || m_iType == CZipActionCallback::cbDelete
		|| m_iType == CZipActionCallback::cbModify || m_iType == CZipActionCallback::cbMoveData || m_iType == CZipActionCallback::cbGet)
...
bool CZipArcDoc::CProgressData::StepIt(ZIP_SIZE_TYPE uStep) // detects not safe stop
{
	if (IsAborting())
		return false;
	if (!IsSubAction())
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		if (iSize == 1 && dlg.m_szComment != szComment || iSize > 1 && !dlg.m_szComment.IsEmpty())
		{
			...
			for (int i = 0; i < iSize; i++)
				m_zip[dw[i]]->SetComment(dlg.m_szComment);
			SetModifiedFlag();
			m_pView->ClearCache();
			m_pView->UpdateItems();
		}
...
BOOL CZipArcDoc::OnNewDocument()
{
	...
	if (b)
	{
		...
		int iMode;
		bool isSplit;
		if (dlg.m_optionsNew.m_bUseSegm)
		{
			isSplit = dlg.m_optionsNew.m_iSegm == 1;			
			if (isSplit)
				iMode = dlg.m_optionsNew.m_bBinarySplit ? CZipArchive::zipCreateBinSplit: CZipArchive::zipCreateSplit;
			else
		...
		if (!OpenZipFile(dlg.m_szFileName, iMode, isSplit ? 0 : dlg.m_optionsNew.m_iVolume))
				b = false;
		else
		{
			SetModifiedFlag();
...
BOOL CZipArcDoc::OpenZipFile(LPCTSTR lpszZipFileName, long iMode, long iVolumeSize) 
{
	...
	do 
	{
		TRY
		{
			...
			m_zip.SetCommitMode(CZipArchive::cmManual);
			CZipPathComponent zpc(lpszZipFileName);
			if (!zpc.GetFileExt().CompareNoCase(_T("jar")))
				m_zip.SetIgnoredConsistencyChecks(CZipArchive::checkCRC);
...
UINT CZipArcDoc::Flush( LPVOID pParam )
{
	...
	CZipArcDoc* pDoc = data.GetDoc();
	CZipArchive& zip = pDoc->m_zip;
	ASSERT((pDoc->IsSegmented() && !pDoc->IsExisting()) || !pDoc->IsSegmented());
	
	TRY
	{
		ZIP_INDEX_TYPE count= 0;
		for (ZIP_INDEX_TYPE i = 0; i < zip.GetCount(); i++)
			if (zip[i]->IsModified())
				count++;
		if (count)
		{	
			pDoc->GetView()->PostMessage(WM_SHOWHIDEVIEW, 0);
			data.m_iType = CZipActionCallback::cbModify;
			data.Begin( count );
			zip.CommitChanges();
			data.Finish();
		}
...
void CZipArcDoc::PrepareData(bool bAll) 
{
	...
	ZIP_INDEX_TYPE iCount = m_zip.GetCount();
	if (bAll || iCount == (ZIP_INDEX_TYPE)m_pView->GetListCtrl().GetSelectedCount())
	{
...
UINT CZipArcDoc::Test( LPVOID pParam )
{
	...
	ZIP_SIZE_TYPE iMax = 0;
	pdw->Sort(true);
	const int iStep = data.GetStepSize();
	INT_PTR i;
	for (i = 0; i < iCount; i++)
	{
	...
	for (i = 0; i < iCount; i++)
	{
		...
		TRY
		{
			...
			{
				data.m_uProcessed = 0;
				data.m_uTotalToProcess = zip.GetFileInfo(nIndex)->m_uUncomprSize;// for GoOn
...
bool CZipArcDoc::AddFolder(CString szFolder, CAddInfoArray &array, CProgressData& data)
{
	...
	while (b)
	{
		...
		{
			ZIP_FILE_USIZE size;
#if defined _ZIP_ZIP64 && defined _MSC_VER >= 1300
			LPWIN32_FIND_DATA fd = ff.GetInfo();
			size = fd->nFileSizeLow | (((ULONGLONG)fd->nFileSizeHigh) << 32);
#else
			size = ff.GetInfo()->nFileSizeLow;
#endif
			files.Add(new CAddInfo(ff.GetFilePath(), size));
		}
		if (data.IsStopping())
			return false;
	}
...
void CZipArcDoc::UpdateUnicodeMode()
{
#if defined _UNICODE && defined _ZIP_UNICODE
	if (m_options.m_iFileNames == 1)
		m_zip.SetUnicodeMode(CZipArchive::umString);
	else if (m_options.m_iFileNames == 2)
		m_zip.SetUnicodeMode(CZipArchive::umExtra);
	else
#endif
		m_zip.SetUnicodeMode(CZipArchive::umNone);
}
...
UINT CZipArcDoc::Add( LPVOID pParam )
{
	...
	for (i = 1; i < iSize; i++) // at 0 is the root path
	{
		...
		ZIP_FILE_USIZE uSize;
		if (isDir)
			uSize = 0;
		else
		{
			HANDLE handle = CreateFile(sz, 0, FILE_SHARE_WRITE | FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, 0);
		...
		CAddInfo* ai = new CAddInfo(sz, uSize, isDir);
		if (ai->m_bDir)
		{
			ai->m_uSize = 0;
			folders.Add(ai);
	...
	ZIP_SIZE_TYPE iMax = 0;
	for (i = 0; i < iSize; i++)
		iMax += (ZIP_SIZE_TYPE)array[i]->m_uSize;
	...
	ASSERT((data.GetDoc()->IsSegmented() && !data.GetDoc()->IsExisting()) || !data.GetDoc()->IsSegmented());
	bool bClearTempPath = false;
	if (data.GetDoc()->IsSegmented() && options.m_bSmart && !options.m_bDefault) // if use not default path for smart compression
	{
	...
	data.GetDoc()->UpdateUnicodeMode();

	if (options.m_iAlgorithm == 1)
		zip.SetCompressionMethod(CZipCompressor::methodBzip2);
	else
		zip.SetCompressionMethod();
	...
	for (i = 0; i < iSize; i++)
	{
		...
		TRY
		{
			...
			if (bAddToReport)
			{
				...
				data.m_uProcessed = 0;
				data.m_uTotalToProcess = (ZIP_SIZE_TYPE)ai->m_uSize;
			}
...
{	
	ASSERT(lpszNewName);
	ZIP_INDEX_TYPE uRealIndex = GetIndex(iItem);
	...
	CZipFileHeader* fh = m_zip[uRealIndex];
	UpdateUnicodeMode();
	fh->SetFileName(lpszNewName);
	if (fh->IsModified())
	{
		SetModifiedFlag();	
	}
	m_pView->SendMessage(WM_REDISPLAY);
}
...
UINT CZipArcDoc::Extract( LPVOID pParam )
{
	...
	for (i = 0; i < iCount; i++)
	{
		...
		TRY
		{
			...
			VERIFY(pFh);
			data.m_uProcessed = 0;
			data.m_uTotalToProcess = pFh->m_uUncomprSize;// for GoOn
			if (!bOverwrite)
			{
				...
				if (ZipPlatform::FileExists(szPredicted))
				{
					...
					if (data.m_iType == CZipActionCallback::cbNothing)
						data.m_iType = CZipActionCallback::cbExtract;
...
void CZipArcDoc::OnCommandSearchSelect() 
{
	...
	if (dlg.DoModal())
	{
		...
		if (m_bSorted)
		{
			...
			{
				ZIP_INDEX_TYPE nIndexToSelect = ar[i];
				for (ZIP_INDEX_TYPE j = 0; j < iCount; j++)
...
bool CZipArcDoc::UpdateReplaceIndex(ZIP_INDEX_TYPE& uReplaceIndex, const CString &szFileName, int& iLastReplaceAction)
{
	
	bool bAdd = true;
	bool bIsSegm = IsSegmented();
	if (m_options.m_bReplace)
	{
		ZIP_INDEX_TYPE idx = m_zip.WillBeDuplicated(szFileName, false);
...
UINT CZipArcDoc::GetFromArchive(LPVOID pParam)
{
	...
	bool bSafeBreak = false;
	ZIP_SIZE_TYPE iMax = 0;
	CZipArray<ZIP_SIZE_TYPE> sizes;
	ZIP_ARRAY_SIZE_TYPE i;
	for (i = 0; i < iSize; i++)
	{
		
		ZIP_INDEX_TYPE uIndex = dw[i];
		...
		else
		{
			...
			if (info.m_bInArchive)
				uEndOffset = info.m_uOffset;
			else
				uEndOffset = (ZIP_SIZE_TYPE)zipFrom.GetStorage()->m_pFile->GetLength();
		}
		ZIP_SIZE_TYPE uSize  = uEndOffset - zipFrom.GetFileInfo(uIndex)->m_uOffset;
		iMax += uSize;
		sizes.Add(uSize);
	}
	
	data.Begin(iMax); 
	data.m_iType = CZipActionCallback::cbGet; 
	data.GetDoc()->GetView()->PostMessage(WM_SHOWHIDEVIEW, 0);
	...
	for (i = 0; i < iSize; i++)
	{
		...
		TRY
		{
			...
			if (bAddToReport)
			{
				...
				data.m_uProcessed = 0;
				data.m_uTotalToProcess = sizes[i];
			}
