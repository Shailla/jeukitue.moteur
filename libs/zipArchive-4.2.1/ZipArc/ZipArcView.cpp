...
void CZipArcView::OnActivateView(BOOL bActivate, CView* pActivateView, CView* pDeactiveView)
{
	...
	if (bActivate)
	{
		...
		CZipArcApp::GetFrame()->ChangeSaveToFinalize(GetDocument()->IsSegmented());
	}
}

LRESULT CZipArcView::OnRedisplay( WPARAM uParam, LPARAM lParam)
{
	CWaitCursor wc;
...
bool CZipArcView::FillAvWithFh(CZipArcViewCache& av, ZIP_INDEX_TYPE idx, bool bFast)
{
...
void CZipArcView::SelectItem(ZIP_INDEX_TYPE iItem, bool bSelect)
{
	if (iItem > INT_MAX)
		return;
...
LRESULT CZipArcView::OnSetreport( WPARAM uParam, LPARAM lParam)
{
...
LRESULT CZipArcView::OnShowhideview( WPARAM uParam, LPARAM lParam)
{
...
LRESULT CZipArcView::OnModifymenu( WPARAM uParam, LPARAM lParam)
{
...
{
	ZIP_INDEX_TYPE iCount = GetDocument()->GetCount();
	pCmdUI->Enable(!m_bHidden && iCount != (ZIP_INDEX_TYPE)GetListCtrl().GetSelectedCount() && iCount);
...
void CZipArcView::SelectArray(CZipIndexesArray &ar)
{
	...
	ZIP_INDEX_TYPE nMin = ZIP_INDEX_TYPE(-1);
	for (ZIP_ARRAY_SIZE_TYPE i = 0; i < iCount; i++)
	{
		ZIP_INDEX_TYPE nIdx = ar[i];
		if (nMin > nIdx)
			nMin = nIdx;
