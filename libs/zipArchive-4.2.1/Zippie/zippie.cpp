...
void FillFromFile(FILELIST& l, LPCTSTR lpszFile, bool bCheck)
{
	...
	for (char* pos = buf; ; pos++)
	{
		...
		if (strncmp(pos, "\n", 1) == 0 || bEnd)
		{
			*pos = '\0';
			CZipString s;
			ZipCompatibility::ConvertBufferToString(s, buf, CP_ACP);
			...
			for (ZIP_ARRAY_SIZE_TYPE k = 0; k < vRevised.GetSize(); ++k)
			{
				...
				else
				{
					if (zip.GetFileInfo(fh, iFile))
						...
						_tprintf(_T("Error while extracting a file. There are troubles with getting info from file number %d\n"), iFile);
					break;
				}
			}
			_tprintf(_T("\n"));
		...
		else if (cmd.HasSwitch(_T("-t")))
		{
			...
			for (ZIP_INDEX_TYPE i = 0; i < iCount; i++)
			{
				...
				{	
					...
					{
						CZipString msg;
						msg.Format(_T("There are troubles with getting info from file number %d"), i);
						lFiles.push_back(msg);
					}
				}
			}
			_tprintf(_T("\n"));
			if (lFiles.size())
			{
				_tprintf(_T("There were errors found in the following files:\n"));
				for (FILELISTIT it = lFiles.begin(); it != lFiles.end(); ++it)
					_tprintf(_T("%s\n"), (LPCTSTR)(*it));
			}
			else
		...
		else if (bIsListing)
		{
			...
			for (ZIP_INDEX_TYPE i = 0; i < iCount; i++)
			{
				...
				if (zip.GetFileInfo(fh, i))
				{
					...
					_tprintf(_T("There are troubles with getting info from file number %d\n"), i);
