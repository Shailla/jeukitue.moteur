...
#define AG_CLIPPED_PIXEL(s, ax, ay) ((ax) < (s)->clip_rect.x || (ax) >= (s)->clip_rect.x+(s)->clip_rect.w || (ay) < (s)->clip_rect.y || (ay) >= (s)->clip_rect.y+(s)->clip_rect.h)
...
#ifdef VIEW_8BPP
# define AG_VIEW_PUT_PIXEL8(dst, c) case 1: *(dst) = (c); break;
#else
# define AG_VIEW_PUT_PIXEL8(dst, c)
#endif

#ifdef VIEW_16BPP
# define AG_VIEW_PUT_PIXEL16(dst, c) case 2: *(Uint16 *)(dst) = (c); break;
#else
...
# define AG_VIEW_PUT_PIXEL24(dst, c) case 3: (dst)[0] = ((c)>>16) & 0xff; (dst)[1] = ((c)>>8) & 0xff; (dst)[2] = (c) & 0xff; break;
# else
# define AG_VIEW_PUT_PIXEL24(dst, c) case 3: (dst)[0] = (c) & 0xff; (dst)[1] = ((c)>>8) & 0xff; (dst)[2] = ((c)>>16) & 0xff; break;
# endif
#else
...
#ifdef VIEW_32BPP
# define AG_VIEW_PUT_PIXEL32(dst, c) case 4: *(Uint32 *)(dst) = (c); break;
#else
# define AG_VIEW_PUT_PIXEL32(dst, c)
#endif

#define AG_VIEW_PUT_PIXEL(p, c) do { switch (agVideoFmt->BytesPerPixel) { AG_VIEW_PUT_PIXEL8((p), (c)) AG_VIEW_PUT_PIXEL16((p), (c)) AG_VIEW_PUT_PIXEL24((p), (c)) AG_VIEW_PUT_PIXEL32((p), (c)) } } while (0)
#define AG_VIEW_PUT_PIXEL2(vx, vy, c) do { Uint8 *_view_dst = (Uint8 *)agView->v->pixels + (vy)*agView->v->pitch + (vx)*agVideoFmt->BytesPerPixel; switch (agVideoFmt->BytesPerPixel) { AG_VIEW_PUT_PIXEL8(_view_dst, (c)) AG_VIEW_PUT_PIXEL16(_view_dst, (c)) AG_VIEW_PUT_PIXEL24(_view_dst, (c)) AG_VIEW_PUT_PIXEL32(_view_dst, (c)) } } while (0)
#define AG_VIEW_PUT_PIXEL2_CLIPPED(vx, vy, c) do { if (!AG_CLIPPED_PIXEL(agView->v, (vx), (vy))) { Uint8 *_view_dst = (Uint8 *)agView->v->pixels + (vy)*agView->v->pitch + (vx)*agVideoFmt->BytesPerPixel; switch (agVideoFmt->BytesPerPixel) { AG_VIEW_PUT_PIXEL8(_view_dst, (c)) AG_VIEW_PUT_PIXEL16(_view_dst, (c)) AG_VIEW_PUT_PIXEL24(_view_dst, (c)) AG_VIEW_PUT_PIXEL32(_view_dst, (c)) } } } while (0)
...
#define AG_PUT_PIXEL2(s, x, y, c) do { AG_PutPixel((s), (Uint8 *)(s)->pixels + (y)*(s)->pitch + (x)*(s)->format->BytesPerPixel, (c)); } while (0)
#define AG_PUT_PIXEL2_CLIPPED(s, x, y, c) do { if (!AG_CLIPPED_PIXEL((s), (x), (y))) { AG_PutPixel((s), (Uint8 *)(s)->pixels + (y)*(s)->pitch + (x)*(s)->format->BytesPerPixel, (c)); } } while (0)

/*
 * Generic alpha blending macros.
 */

#define AG_BLEND_RGBA(s, p, r, g, b, a, m) AG_BlendPixelRGBA((s),(p),(r),(g),(b),(a),(m))
#define AG_BLEND_RGBA2(s, x, y, r, g, b, a, m) do { AG_BlendPixelRGBA((s), (Uint8 *)(s)->pixels + (y)*(s)->pitch + (x)*(s)->format->BytesPerPixel, (r),(g),(b),(a),(m)); } while (0)
#define AG_BLEND_RGBA2_CLIPPED(s, x, y, r, g, b, a, m) do { if (!AG_CLIPPED_PIXEL((s), (x), (y))) { AG_BlendPixelRGBA((s), (Uint8 *)(s)->pixels + (y)*(s)->pitch + (x)*(s)->format->BytesPerPixel, (r),(g),(b),(a),(m)); } } while (0)
...
__BEGIN_DECLS
extern DECLSPEC AG_ObjectClass agDisplayClass; 
extern DECLSPEC AG_Display *agView; 
extern DECLSPEC AG_PixelFormat *agVideoFmt; 
extern DECLSPEC AG_PixelFormat *agSurfaceFmt; 
extern DECLSPEC int agRenderingContext; 
extern DECLSPEC const char *agBlendFuncNames[]; 
extern DECLSPEC AG_ClipRect *agClipRects; /* Clipping rectangle stack (first entry always covers whole view */ extern int agClipStateGL[4]; 
extern DECLSPEC Uint agClipRectCount;
extern DECLSPEC const SDL_VideoInfo *agVideoInfo; 
extern DECLSPEC int agFullscreenMode; 
extern DECLSPEC int agAsyncBlits; 
extern DECLSPEC int agGUI; 
extern DECLSPEC int agInitedSDL; 
extern DECLSPEC int agInitedSDLVideo; 
extern DECLSPEC int AG_InitVideo(int, int, int, Uint);
extern DECLSPEC int AG_InitVideoSDL(SDL_Surface *, Uint);
extern DECLSPEC int AG_InitGUI(Uint);
extern DECLSPEC void AG_DestroyGUI(void);
extern DECLSPEC void AG_QuitGUI(void);
extern DECLSPEC void AG_ClearBackground(void);
extern DECLSPEC int AG_SetRefreshRate(int);
extern DECLSPEC void AG_ViewUpdateFB(const AG_Rect2 *);
extern DECLSPEC void AG_ViewVideoExpose(void);
extern DECLSPEC int AG_ResizeDisplay(int, int);
extern DECLSPEC void AG_DestroyVideo(void);
extern DECLSPEC void AG_BeginRendering(void);
extern DECLSPEC void AG_EndRendering(void);
extern DECLSPEC int AG_ProcessEvent(SDL_Event *);
extern DECLSPEC void AG_SetVideoResizeCallback(void (*)(Uint, Uint));
extern DECLSPEC void AG_BindGlobalKey(SDLKey, SDLMod, void (*)(void));
extern DECLSPEC void AG_BindGlobalKeyEv(SDLKey, SDLMod, void (*)(AG_Event *));
extern DECLSPEC int AG_UnbindGlobalKey(SDLKey, SDLMod);
extern DECLSPEC void AG_ClearGlobalKeys(void);
extern DECLSPEC void AG_ViewAttach(void *);
extern DECLSPEC void AG_ViewDetach(struct ag_window *);
extern DECLSPEC struct ag_window *AG_FindWindow(const char *);
extern DECLSPEC AG_Surface *AG_DupSurface(AG_Surface *);
extern DECLSPEC int AG_ScaleSurface(AG_Surface *, Uint16, Uint16, AG_Surface **);
extern DECLSPEC void AG_SetAlphaPixels(AG_Surface *, Uint8);
extern DECLSPEC int AG_DumpSurface(AG_Surface *, char *);
extern DECLSPEC void AG_ViewCapture(void);
extern DECLSPEC void AG_FlipSurface(Uint8 *, int, int);
#ifdef HAVE_OPENGL
extern DECLSPEC Uint AG_SurfaceTexture(AG_Surface *, float *);
extern DECLSPEC void AG_UpdateTexture(AG_Surface *, int);
extern DECLSPEC AG_Surface *AG_CaptureGLView(void);
#endif
extern DECLSPEC void AG_EventLoop_FixedFPS(void);
#define AG_EventLoop() AG_EventLoop_FixedFPS()
extern DECLSPEC Uint8 AG_MouseGetState(int *, int *);
#ifdef AG_DEBUG
extern DECLSPEC struct ag_window *AG_EventShowPerfGraph(void);
extern DECLSPEC struct ag_window *AG_GuiDebugger(void);
#endif
extern DECLSPEC void AG_BlendPixelRGBA(AG_Surface *, Uint8 *, Uint8, Uint8, Uint8, Uint8, AG_BlendFn);
...
extern DECLSPEC AG_Surface *AG_SurfaceNew(Uint, Uint, AG_PixelFormat *, Uint);
...
extern DECLSPEC AG_Surface *AG_SurfaceFromPixelsRGB(void *, Uint, Uint, int, int, Uint32, Uint32, Uint32);
extern DECLSPEC AG_Surface *AG_SurfaceFromPixelsRGBA(void *, Uint, Uint, int, int, Uint32, Uint32, Uint32, Uint32);
extern DECLSPEC AG_Surface *AG_SurfaceFromSDL(SDL_Surface *);
extern DECLSPEC SDL_Surface *AG_SurfaceToSDL(AG_Surface *);
extern DECLSPEC AG_Surface *AG_SurfaceFromSurface(AG_Surface *, AG_PixelFormat *, Uint);
extern DECLSPEC AG_Surface *AG_SurfaceFromBMP(const char *);
extern DECLSPEC void AG_SurfaceCopy(AG_Surface *, AG_Surface *);
