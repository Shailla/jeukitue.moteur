...
bool BrancheIterator::operator++() {
	...
	// Essaie de descendre, sinon recule jusqu'� pouvoir descendre ou avoir fini
	while(_br.size()) {
		pos = _pos.top();
		size = _size.top();
		br = _br.top();
...
Branche* Branche::getSubBrancheByIdOrDistantTmpId(DistantTreeProxy* distant, int brancheId) {
	if(brancheId >= 0) {
...
Branche* Branche::getSubBrancheByDistantTmpId(DistantTreeProxy* distant, int brancheTmpId) {
...
Valeur* Branche::getValeurByDistantTmpId(DistantTreeProxy* distant, int valeurTmpId) {
...
Branche* Branche::createSubBrancheForClient(const string& brancheName, int revision) {
	...
	// V�rifie si la branche n'existe pas d�j�
	...
	if(brancheExisting) {
		...
		message << "La branche nomm�e '" << brancheName << "' de parent " << CollectionsUtils::toString(getBrancheFullId()) << " existe d�j�";
	...
	_subBranchesByTmpId[tmpRef] = newBranche;		// Moins 1 pour �viter les collisions avec les ID non temporaires
...
Branche* Branche::createSubBrancheForServer(DistantTreeProxy* distant, const string& brancheName, DONNEE_TYPE type, int revision) {
	// Alloue une r�f�rence pour la nouvelle branche
	int ref = _brancheRefGenerator.genRef();	// On d�marre � 1
	...
	if(parentType == DONNEE_PUBLIC) {		// Sur une branche serveur on peut cr�er une branche locale ou serveur
	...
	else if(parentType == DONNEE_PRIVATE || parentType == DONNEE_PRIVATE_SUB) {	// Sur une branche client on ne peut cr�er qu'une donn�e client fille
		resultType = DONNEE_PRIVATE_SUB;
	}
	else if(parentType == DONNEE_LOCAL) {	// Sur une branche local on ne peut cr�er qu'une donn�e local
...
Branche* Branche::addSubBranche(DistantTreeProxy* distant, int brancheId, DONNEE_TYPE type, const std::string& brancheName, int brancheRevision) {
	...
	if(iter != _subBranchesById.end()) {
		cerr << endl << __FILE__ << ":" << __LINE__ << "La branche " << brancheId << " (" << brancheName << ") de parent " << CollectionsUtils::toString(getBrancheFullId()) << " existe d�j�";
...
Branche* Branche::acceptTmpSubBranche(DistantTreeProxy* distant, int brancheTmpId, int brancheId, int brancheRevision) {
...
Valeur* Branche::acceptTmpValeur(DistantTreeProxy* distant, int valeurTmpId, int valeurId, int valeurRevision) {
...
Valeur* Branche::createValeurForClient(UPDATE_MODE updateMode, const string& valeurName, int revision, const AnyData& valeur) {
	...
	// D�termine le type de la nouvelle valeur � cr�er
	DONNEE_TYPE resultType;
	DONNEE_TYPE parentType = getDonneeType();
...
Valeur* Branche::createValeurForServeur(DistantTreeProxy* distant, UPDATE_MODE updateMode, const string& valeurName, int revision, const AnyData& valeur) {
	// Alloue une r�f�rence pour la nouvelle branche
	int ref = _valeurRefGenerator.genRef();		// On d�marre � 1
...
Valeur* Branche::addValeur(DistantTreeProxy* distant, UPDATE_MODE updateMode, int valeurId, const string& valeurName, int valeurRevision, const jkt::AnyData& valeur) {
	...
	if(newValeur) {
		cout << endl << __FILE__ << ":" << __LINE__ << "La valeur " << valeurId << " (" << valeurName << ") de parent " << CollectionsUtils::toString(getBrancheFullId()) << " existe d�j�";
	}
	else {
		// D�termine le type de la nouvelle valeur � cr�er
		DONNEE_TYPE resultType;
...
void Branche::print(ostringstream& out, DistantTreeProxy* distant, bool tmpId, bool publicOnly, int indentation) {
	...
	case DONNEE_DEFAULT:
		out << "+err_d";	// Une donn�e ne devrait jamais �tre de ce type, elle est cr��e avec pour recevoir automatiquement son vrai type
		break;
	case DONNEE_PUBLIC:
		out << "+pub";
		break;
	...
	if(!publicOnly || getDonneeType() != DONNEE_PRIVATE) {
		// Affiche les valeurs de la branche de mani�re ordonn�e
