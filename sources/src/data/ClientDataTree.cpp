...
void ClientDataTree::collecteChangements(vector<Changement*>& changements) {
	...
	while(++it) {
		...
		// NOUVELLE BRANCHE : branche pr�sente sur le client dont le serveur n'a pas encore connaissance (donc avec un identifiant temporaire)
		...
		for(Valeur* valeur : valeurs) {
			...
			// NOUVELLE VALEUR : valeur pr�sente sur le client dont le serveur n'a pas encore connaissance (donc avec un identifiant temporaire)
			...
			// UPDATE VALEUR : Une valeur a chang� et le client n'en a pas encore connaissance
...
void ClientDataTree::receiveChangementsFromServer() {
	try {
		...
		/* ********************************************
		 * Traitement des donn�es venant du serveur
		 * *******************************************/
		...
		while((fromServer = interlocutor->popDataReceived())) {
			...
			for(itCh = changements.begin() ; itCh != changements.end() ; itCh++) {
				...
				try {
					// Le serveur accepte la cr�ation de la nouvelle branche demand�e par ce client et lui attribue son identifiant d�finitif
					...
					// Le serveur informe de la cr�ation d'une nouvelle branche
					...
					// Le serveur informe de la cr�ation d'une nouvelle valeur
					...
					// Le server confirme la r�vision de la valeur qu'il a re�u
					...
					// Le server confirme la r�vision de la branche qu'il a re�u
		...
		// Envoi les changements pr�sents sur le client au serveur
