...
Branche* ServeurDataTree::getBrancheFromDistant(DistantTreeProxy* distant, const vector<int>& brancheId) {
...
Valeur* ServeurDataTree::getValeurFromDistant(DistantTreeProxy* distant, const vector<int>& brancheId, int valeurId) {
	vector<int>::const_iterator iter;
...
Donnee* ServeurDataTree::initDonneeAndMarqueurFromDistant(DistantTreeProxy* client, Donnee* donnee, int donneeClientTmpId) {
	...
	case DONNEE_PRIVATE:
	{
		...
		// Ajoute un marqueur pour chaque distant et met l'identifiant temporaire dans le distant du client qui a cr�� la donn�e
	...
	case DONNEE_PRIVATE_SUB:
		if(!client) {
			cerr << endl << "Initialisation de donn�e impossible sans client sp�cifi� sur branche priv�e";
		}
		else if(contains<DistantTreeProxy*>(_clients, client)) {
			client->addMarqueur(donnee, donneeClientTmpId);
		}
		else {
			cerr << endl << "Initialisation de donn�e impossible car client inconnu sur branche priv�e";
		}

		break;
	case DONNEE_LOCAL:
		// Pas besoin d'initialiser les marqueurs en mode local ils ne serviront � rien
		break;
	default:
		cerr << endl << "Donn�e � initialiser de type inconnu";
		break;
	}
	...
	try {
		...
		for(DistantTreeProxy* client : getDistants()) {
			if(now - client->getUpdateServerToClientTime() >= (Uint32)client->getControl().getUpdateServerToClientDelay()) {
				...
				// Priorise et filtre les changements qui seront vraiment envoy�s
...
void ServeurDataTree::collecteChangements(vector<Changement*>& changements, DistantTreeProxy* distant) {
	...
	while(++it) {
		...
		// NOUVELLE BRANCHE : branche pr�sente sur le serveur mais dont le client n'a pas connaissance
		...
		for(Valeur* valeur : valeurs) {
			...
			// NOUVELLE VALEUR : valeur pr�sente sur le serveur mais dont le client n'a pas connaissance
			if(marqueur->getConfirmedRevision() == MarqueurDistant::MARQUEUR_REVISION_INIT) {
			...
			// VALEUR MODIFIEE : valeur pr�sente sur le serveur dont le client a connaissance mais qui a chang�
...
Branche* ServeurDataTree::getBrancheByDistantTmpId(DistantTreeProxy* distant, const vector<int>& parentBrancheId, int brancheTmpId) {
	vector<int>::const_iterator iter;
...
Valeur* ServeurDataTree::getValeurByDistantTmpId(DistantTreeProxy* distant, const vector<int>& brancheId, int valeurTmpId) {
	vector<int>::const_iterator iter;
	...
	try {
		...
		for(it = _clients.begin() ; it != _clients.end() ; it++) {
			...
			while((data = interlocutor->popDataReceived())) {
				...
				// Tri des changements de mani�re � les effectuer en commen�ant par ceux qui sont le plus proche de la racine, puis les second niveau, puis etc...
				...
				for(itCh = changements.begin() ; itCh != changements.end() ; itCh++) {
					try {
						...
						// Le client confirme la r�vision de branche qu'il a re�u
						...
						// Le client confirme la r�vision de valeur qu'il a re�u
						...
						// Le client demande la cr�ation d'une nouvelle branche
						else if(AddBrancheFromClientChangement* chgt = dynamic_cast<AddBrancheFromClientChangement*>(*itCh)) {
							Branche* branche;

							try {
								// Si la branche existe d�j� on renvoie juste la r�ponse au client
								branche = getBrancheByDistantTmpId(distant, chgt->getParentBrancheId(), chgt->getBrancheTmpId());
							}
							catch(NotExistingBrancheException& exception) {
								// Si la branche n'existe pas encore on la cr�e
								cout << endl << "Distant '" << distant->getInterlocutor()->getName() << "' => La branche de parent " << CollectionsUtils::toString(chgt->getParentBrancheId()) << " et d'id temportaire " << chgt->getBrancheTmpId() << " n'existe pas encore, on va la cr�er" << flush;
						...
						// Le client demande la cr�ation d'une nouvelle valeur
						else if(AddValeurFromClientChangement* chgt = dynamic_cast<AddValeurFromClientChangement*>(*itCh)) {
							Valeur* valeur;

							try {
								// Si la valeur existe d�j� on renvoie juste la r�ponse au client
						...
						// Le client demande la modification d'une valeur
							...
							if(valeur->getRevision() < chgt->getRevision()) {
								...
								LOGERROR((" Exception : La r�vision client est plus ancienne que celle du serveur"));
							}
						}
