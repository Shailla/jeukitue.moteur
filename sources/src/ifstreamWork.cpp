...
ifstreamWork::ifstreamWork() {
	...
	next = 0;		// Pas de sous-fichier par d�faut
	txt = NULL;
}
...
bool ifstreamWork::open( const string &nomFichier ) {
	...
	if( string( nomFichier.end()-4,nomFichier.end() ) == ".map" ) {		// Inclusion d'un fichier map
		nomComplet += nomFichier;

		txt = new char[ nomComplet.length() ];
		strcpy( txt, nomComplet.c_str() );

		current.open( nomComplet.c_str() );		// Ouverture du fichier associ�
...
/*	if( next )	// Si un (ou plusieurs) sous-fichier est ouvert
	{
		if( next->eof() )	// Si ce(s) sous-fichier(s) est ferm�
			return bFin;	// Indique si on est aussi � la fin du fichier courrant
		else
			return false;	// S'il(s) est pas ferm� indique qu'on a pas fini de lire les donn�es
	}
	else
	{
		return bFin;
