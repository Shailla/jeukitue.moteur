...
void CDlgBoite::afficheCadre()
{
	...
	// Affiche le fond de la bo�te en mode GL_QUADS, puis les contours en mode GL_LINE_LOOP
	for( int i=0 ; i<2 ; i++ )
	{
		if( i==0 )
		{
		...
		glBegin( rend );
			// Affichage du cadre � proprement dit
	...
	// Affiche la barre de titre de la bo�te de dialogue
	glBegin( GL_QUADS );
			glVertex2f( CORX + TAILLEBX,	CORY + TAILLEBY			);
...
void CDlgBoite::go() {
	afficheCadre();		// Affiche le cadre de la bo�te de dialogue
	afficheTitre();		// Affiche le titre dans le cadre de la bo�te de dialogue
	afficheIcone();		// Affiche l'icone associ� � la boite de dialogue
	afficheTexte();		// Affiche le texte de la bo�te de dialogue
	afficheBouton();	// Affiche les bouton dans la bo�te de dialogue
}

void CDlgBoite::right() {
	bool cbon = false;	// Indique que le focus a �t� d�plac� avec succ�s
