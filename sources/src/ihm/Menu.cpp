...
CMenu::CMenu(const char *newTitle, const char **newItems, int nbrItems, PF *fct_suivante, PF fct_retour, void **liste_arg, PFV fct_refresh ) : CDlg() {
	...
	items = new char*[ nbrItems ];		// Cr�tion et copie des items du menu
	...
	fonction_retour = fct_retour;	// Fonction � ex�cuter au retour du menu

	bItemsDroits = false;		// Indique qu'aucun item droit n'et s�lectionn�
	items_droits = 0;			// Pas d'items droits par d�faut
	items_rem = 0;				// Pas d'items remarques par d�faut
	mode = 0;					// Mode normal
...
void CMenu::afficheItems() {
	...
	// Affichage de la barre de d�filement
	if( nbrChoixY > MAXITEM ) {	// S'il y a beaucoup d'items � afficher
	...
	while( i<MAXITEM && i+ajust<nbrChoixY ) {
		if( i+ajust==choixY )
			glColor3f( 1.0, 0.0, 0.0 );		// Couleur rouge si focuss�
		else if( fonction_suivante[ i+ajust ] == 0 )
			glColor3f( 0.6f, 0.6f, 0.6f );
		else
			glColor3f( 1.0, 1.0, 1.0 );		// Couleur blanche par d�faut
	...
	if( items_droits ) {
		...
		while( i<MAXITEM && i+ajust<nbrChoixY )
		{
			if( fonction_suivante[ i+ajust ]==0 )
				...
				glColor3f( 0.0, 1.0, 0.0 );		// Couleur blanche par d�faut
			else
				glColor3f( 1.0, 1.0, 1.0 );		// Couleur rouge si focuss�
...
void CMenu::afficheItemsRem() {
	if( items_rem ) {
		// Affichage de la remarque associ�e � l'item focuss�
...
void CMenu::go() {
	if( fct_Refresh )	// S'il y a une fonction de rafraichissement pour ce menu, ex�cute-la
		fct_Refresh();
	...
	afficheItemsRem();		// Affiche la remarque associ�e � l'item
}
...
void CMenu::add_ItemsDroits(int num, const char *txt) {	// Impl�mente le num� item droit
	if( !items_droits ) {
...
void CMenu::add_ItemsRem(int num, char *txt) {	// Impl�mente le num� item droit
	if( !items_rem ) {
