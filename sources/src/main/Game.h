...
public:
	...
	bool getGravite() const;		// Indique si la gravitÃ© est active
	void setGravite(bool gravite);

	// Gestion de la Map
	jkt::CMap *getMap();
	void changeActiveMap(jkt::CMap* map);
	bool openMap( const std::string &nomFichierMap ) noexcept(false);
	...
	// Gestion du serveur
	void setModeServer();								// CrÃ©e la classe Server pour un jeu en mode serveur
	...
	// Gestion des joueurs
	void deletePlayers();							// DÃ©truit tous les joueurs
	...
	/**
	 * Ajout un joueur Ã  la partie
	 * @param player joueur Ã  ajouter
	 * @return index du joueur ou -1 si l'ajout a Ã©chouÃ©
	 */
...
public:
	...
	void afficheViseur(int x, int y) const;			// Affiche le viseur Ã  la position voulue sur l'Ã©cran
