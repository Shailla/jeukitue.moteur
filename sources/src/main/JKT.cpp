...
extern JktSon::CDemonSons *DemonSons;	// Requ�tes des sons � jouer
...
void play_handle_key_down( SDL_Event *event ) {
	...
	case SDL_KEYDOWN:
	{
		...
		case SDLK_g :		// D�sactive / active la gravit� (et annule la vitesse)
			if( Game.getMap() )
				Game.Gravite( !Game.Gravite() );
			break;

		case SDLK_v :		// D�sactive / active la gravit� (et annule la vitesse)
			if( Game.getMap() )
...
bool deprecatedOpenMAP(const void *nomFichier) {
	...
	if( !Game.openMap( nomFichierMap ) ) {
		cerr << endl << __FILE__ << ":" << __LINE__ << " Erreur � l'ouvertur du fichier Map : " << nomFichierMap;
		return false;
	}
...
void boucle() {
	...
	for( ;; ) {
		...
		if(serveurDataTree) {
			serveurDataTree->receiveChangementsFromClients();		// Le serveur re�oit les changements des donn�e du jeu de la part des clients
			serveurDataTree->diffuseChangementsToClients();			// Diffuse les changements des donn�es du jeu aux clients
		}
