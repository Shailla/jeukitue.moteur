...
void quit_JKT() {
	...
	string trace = "Deni�re erreurs des librairies :";
...
bool checkEventsForIntro(void)		// V�rifie si 'escape' ou le bouton souris ont �t� frapp�s
        ...
        switch( event.type ) {
 		case SDL_MOUSEBUTTONDOWN:	// Handle mouse button presses
			if( event.button.button==SDL_BUTTON_LEFT )			// Si le bouton de gauche
				if( event.button.type==SDL_MOUSEBUTTONDOWN )	// est pr�ss�
					return true;
...
CSon* sonChariot;		// Son retour chariot machine � �crire
CSon* sonTouche;		// Son frappe d'une touche clavier
CSon* sonEspace;		// Son frappe de la touche espace clavier
CSon* sonHurlement;		// Son hurlement du sauveur de la plan�te
...
void load_IntroSub(const int width, const int height) {
	 ...
	 * lettre par lettre (genre machine � �crire)
	 * ***********************************************/
	...
	for( unsigned int i=0 ; i<str1.length() ; i++ ) {
		...
		SDL_GL_SwapBuffers();	// Echange des buffers graphique -> affiche � l'�cran
		SDL_Delay( (unsigned int)( 60.0f * ((float)rand()/(float)RAND_MAX) ) );	// Petite attente entre deux lettres cons�cutives

		if( checkEventsForIntro() ) {	// V�rifie si l'utilisateur veut sortir de l'intro
			delete image2;
			return;
		}
	}

	// Attends un petit instant � la fin de l'affichage du texte complet
	SDL_Delay( 1500 );
	...
	for( int i=0 ; i<20 ; i++ ) {		// Laisse quelques instants l'image affich�e
		SDL_Delay( 100 );
	...
	for(unsigned int i=0 ; i<str2.length() ; i++) {
		...
		SDL_GL_SwapBuffers();	// Echange des buffers graphique -> affiche � l'�cran
		SDL_Delay( 75 + (unsigned int)( 60.0f * ((float)rand()/(float)RAND_MAX) ) );	// Petite attente entre deux lettres cons�cutives
