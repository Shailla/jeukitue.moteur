...
void Controller::executeAction(AG_Event *event) {
	...
	if(action >= 10000) {	// Au del� de 10000 il s'agit d'une action d'un plugin, on la redispatche � tous les plugins
		...
		case ShowOpenAseViewAction:
			...
			// Ouvre l'�cran de choix d'une Map en mode de jeu local
		...
		case PingServerAction:
		{
			...
			if(client) {
				client->sendNotConnectedRequestPingToServer(Config.Reseau.getIpServer(), Config.Reseau.getServerPort());				// Demande au serveur de r�pondre � un ping
			}
			else {
				AG_TextMsg(AG_MSG_ERROR, "Action impossible, vous n'etes pas connecte");
			}
		...
		case ConnectClientAction:
		{
			if( !_networkManager->ouvreClient(Config.Reseau.getIpServer(), Config.Reseau.getServerPort(), Config.Reseau.getServerPortTree()) ) {
				_networkManager->setOn( false );		// Signale que le r�seau ne peut pas �tre utilis�
		...
		// Affiche les donn�es de la MAP courante sous forme d'un arbre
		...
		// Affiche la deni�re erreur OpenGL dans une popup
		...
		// Demande de rafraichissement de la vue pass�e en param�tre
		case Refresh:
		{
