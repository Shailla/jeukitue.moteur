...
public:
	...
	bool openInClientMode(const std::string &remIp, Uint16 remPort);	// Ouverture en mode client
	...
	void add( const std::string& txt );					// Ajoute du texte à la fin du paquet
	...
	void readString( std::string& mot );					// Lit le texte pointé vers un string
	void readRecapFromServer( const CPlayer &player );

	std::string iPInToString();					// Renvoie l'IP in en string
	std::string iPOutToString();					// Renvoie l'IP out en string
	std::string debugToString();							// Renvoie un résumé du paquet sous forme de string
