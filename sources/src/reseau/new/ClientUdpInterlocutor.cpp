	...
	while(STOPPED != getConnexionStatus() && STOPPING != getConnexionStatus()) {
		...
		while((msg = _interlocutor->popTechnicalMessageReceived())) {
			...
			if(techMsg) {
				...
				default:
					stringstream message;
					message << "Reception de donnees techniques inconnues ==> ignor�";
					log(__LINE__, __FILE__, message);
					break;
				}
				...
				stringstream message;
				message << "Reception de donnees inconnues ==> ignor�";
				log(__LINE__, __FILE__, message);
			}
		}
...
void ClientUdpInterlocutor::sendingProcess() {
	...
	while(STOPPED != getConnexionStatus() && STOPPING != getConnexionStatus()) {
		...
		// Envoi les messages techniques si on n'est pas arr�t�, sinon purge les
		...
		// Envoi les messages de donn�es si on est connect�, sinon purge les
		while((data = _interlocutor->popDataToSend())) {
			if(CONNECTED == getConnexionStatus()) {
				// Pour un message de donn�es on �crit explicitement le code DATA en d�but de flux
				_packetOut->len = data->size() + 2;

				SDLNet_Write16(TechnicalMessage::TECHNICAL_MESSAGE::DATA, _packetOut->data);	// Message de donn�es
...
void ClientUdpInterlocutor::receiveOnePacket() {
	...
	if(packetReceived) {
		...
		if(size >= 2) {
			...
			stringstream message;
			message << "Message inconsistant from " << IpUtils::translateAddress(_packetIn->address) << " ==> ignor�";
			log(__LINE__, __FILE__, message);
		}
	}
}
