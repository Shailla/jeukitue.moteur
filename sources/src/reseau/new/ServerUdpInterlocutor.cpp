	...
	if(CONNECTED == getConnexionStatus() && packetReceived) {
		...
		if(_clientsOfServer.find(address) == _clientsOfServer.end()) {
			...
			if(size >= 2) {
				...
				stringstream message;
				message << "Message inconsistant et client inconnu from " << IpUtils::translateAddress(address) << " ==> ignor�";
				log(__LINE__, __FILE__, message);
			}
		}
		else {
			...
			if(size >= 2) {
				...
				stringstream message;
				message << "Message inconsistant de client connu from " << IpUtils::translateAddress(address) << " ==> ignor�";
				log(__LINE__, __FILE__, message);
			}
		}
	}
