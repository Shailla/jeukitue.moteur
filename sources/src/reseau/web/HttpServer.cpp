	...
	while(1) {
		...
		for(TcpPacket* tcpPacket : tcpPackets) {
			...
			try {
				...
				/* ****************************************** */
				/* R�conciliation de la session HTTP          */
				/* ****************************************** */
				...
				if(httpRequest->getStatus() == HttpRequest::PINCOMPLET) {
					...
					LOGERROR(("Erreur de lecture de la requ�te HTTP"));
				...
				case HttpException::RESOURCE_NOT_FOUND_EXCEPTION:
					...
					if(resource) {
						...
						LOGERROR(("On ne devrait jamais �tre ici (fichier d'erreur HTML manquant)"));
					}
				...
				case HttpException::MALFORMED_HTTP_REQUEST:
					LOGWARN(("Requ�te http malform�e re�ue sur le service : '%s'", endpoint.c_str()));
				...
				default:
					LOGERROR(("On ne devrait jamais �tre ici (erreur non-ma�tris�e) '%s' : %d", tcpPacket->getData().c_str(), exception.getCode()));
					break;
				}
			}
			catch(MalformedJsonException& exception) {
				LOGWARN(("La requ�te HTTP re�ue contient un Json malform� sur le service : '%s'", endpoint.c_str()));
				...
				if(resource) {
					...
					LOGERROR(("On ne devrait jamais �tre ici (fichier d'erreur HTML manquant) '%s' : %d", endpoint.c_str(), exception));
				}
			}
