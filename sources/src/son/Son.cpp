...
LOGDEBUG(("CSon::~CSon() nom=%s%T", nom.c_str(), this ));
	...
	set<CReqSon*>::iterator p;		// Destruction de toutes les requ�tes sur ce son
	for( p = m_TabReq.begin() ; p != m_TabReq.end() ; p++ )
	{
		pDemon->Erase( *p );		// Supprime la requ�te de la liste appartenant au d�mon
		delete *p;
	}
	m_TabReq.clear();	// Vide la liste des requ�tes du son (pas indispensable vu qu'elle va �tre d�truite)

	if( !m_Sample )		// Destruction des �chantillions du son
