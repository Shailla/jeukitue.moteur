...
CGeoMaker::CGeoMaker(CMap *map) {
	...
	m_OffsetMateriau = -1;
	m_bMaterialTexture = false;	// "true" si l'objet est associ� � un mat�riau
	m_MaterialRef = 0;		// R�f�rence du mat�riau associ�
	...
	m_TabSubMat = NULL;			// Index des �ventuels sous-mat�riau
...
void CGeoMaker::setVertex(int nbr, float* tab)	// Donn�es du tableau des sommets
{
	m_NumVertex = nbr;
	m_TabVertex = tab;
}

void CGeoMaker::setFaces(int nbr, int* tab)		// Donn�es du tableau des index de faces
{
	if(m_TabFaces) {
		delete[] m_TabFaces;
	}
...
CTextureMaterialGeo* CGeoMaker::makeTextureMaterialGeo(CMaterialTexture* mat) {
	// Enl�ve l'indexation des vertex
	lineariseVertex();

	// Enl�ve l'indexation des sommets de texture
	lineariseTexVertex();

	// R�cup�re et copie le canal de texture unique
...
CMultiMaterialGeo* CGeoMaker::makeMultiMaterialGeo(CMaterialMulti* mat) {
	...
	// Lin�arise les sommets
	lineariseVertex();			// Enl�ve l'indexation des vertex

	// Lin�arise les coordonn�es de texture de tous les canaux
	lineariseTexVertex();
	...
	// Cr�ation de l'objet g�om�trique
...
void CGeoMaker::optimiseSubMat(map<int,int> &canauxnumbers) {	// Rassemble les faces associ�s au m�me sous-mat�riau
	float* vertexRes = new float[m_NumVertex*3];
	float* normalsRes = 0;
	if(m_TabVectNormaux)
		normalsRes = new float[m_NumVertex*3];

	vector<int> listeSousMateriaux;	// Liste des num�ros de sous-mat�riaux

	// Liste les sous-mat�riaux
	for(int i=0; i<m_NumFaces; i++) {
		bool indic = true;

		vector<int>::iterator iter = listeSousMateriaux.begin();

			// V�rifie si le sous-mat�riau est d�j� dans la liste
		for( ; iter!=listeSousMateriaux.end(); iter++) {
			if((*iter) == m_TabSubMat[i])	// S'il y est
				indic = false;			// ne pas l'ajouter � la liste
		}

		if(indic)	// Si le mat�riau n'est pas encore dans liste
	...
	map<int,vector<int> > canalordre;	// First = canal
										// Second = liste des index de sommets associ�s

	vector<int>::iterator iter = listeSousMateriaux.begin();
	for( ; iter!=listeSousMateriaux.end() ; iter++) {
		int sousmat = *iter;	// Sous-mat�riau
	...
	// Initialisation de la liste des coordonn�es de texture
	...
	for( ; iterMap!=canalordre.end() ; iterMap++) {
		...
		for( ; iterOrdre!=ordre.end() ; iterOrdre++) {
			...
			// D�placements des 3 vecteurs normaux de la face de la face
			if(normalsRes) {
			...
			// D�placement des coordonn�es de texture
		...
		// Nombre de sommets concern� par le canal de texture
		canauxnumbers[canal++] = (int)ordre.size();
	}

	assert(index == m_NumFaces);

	// Copie des r�sultats dans les tableaux et suppression des tableaux temporaires
...
void CGeoMaker::lineariseVertex() {		// Renvoie le tableau de vertex sous forme non-index�e
	if(m_TabFaces) {
		m_NumVertex = m_NumFaces*3;
		float* vertex = new float[3*m_NumVertex];

			// Lin�arise les vertex � partir du tableau des indices et des vertex index�s
...
CSimpleGeo* CGeoMaker::makeSimpleGeo() {
	...
	geo = new CSimpleGeo(m_Map,m_Nom,m_NumVertex,m_TabVertex,m_NumFaces,m_TabFaces,m_Color,m_bSolid);
		...
		// Oublie les �l�ments qui ont �t� transmis � l'instance de CSimpleGeo
		// afin qu'ils ne soient pas supprim�s par l'instance de CGeoMaker
	m_TabVertex = NULL;
	m_TabFaces = NULL;
...
void CGeoMaker::LitSousMateriaux(TiXmlElement* el, map<int, int>& canauxNumbers) throw(CErreur) {
	...
	for( ; elC!=0 ; elC=elC->NextSiblingElement()) {
		...
		canauxNumbers.insert(pair<int, int>((int)var1, (int)var2));
	}

	if(canauxNumbers.size() != nbr) {
