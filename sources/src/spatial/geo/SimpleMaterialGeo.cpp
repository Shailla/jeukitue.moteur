...
CSimpleMaterialGeo::CSimpleMaterialGeo(CMap* map) : MapObject(map, MapObject::GEO) {
	m_NumVertex = 0;
	m_TabVertex = NULL;			// Pointeur sur le tableau de sommets
	m_TabVectNormaux = NULL;
...
void CSimpleMaterialGeo::setMaterial(const string& matRef) {
	...
	if(!mat) {
		stringstream txt;
		txt << "Erreur : L'objet '" << matRef << "' devrait être un CMaterial";
		throw CErreur(txt);
	}
...
void CSimpleMaterialGeo::gereContactPlayer(float positionPlayer[3], CPlayer *player, float deltaTime) {
