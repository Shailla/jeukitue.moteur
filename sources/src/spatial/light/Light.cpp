...
int CLight::m_nbrLightActivated = 0;	// Nombre de lumi�res actuellement activ�es
...
void CLight::translate( float x, float y, float z )	// Translation pond�r�e selon X, Y et Z de l'objet
{
	m_Position[ 0 ] += x;
	m_Position[ 1 ] += y;
	m_Position[ 2 ] += z;
...
void CLight::Desactive() {
	...
	freeLight( m_refLight );		// Lib�re la lumi�re
}
