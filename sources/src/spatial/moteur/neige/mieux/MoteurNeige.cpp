...
bool MoteurNeige::Lit(TiXmlElement* el, CMap& map, MapLogger* mapLogger) throw(CErreur) {
	...
	if(!el->Attribute(Xml::NBR_PARTICULES_ON_GROUND, &nbrParticulesOnGround)) {		// Lecture nom de la Map � importer
		mapLogger->logInfo("Fichier Map : Nombre de particules du moteur sur le sol manquant absent, application valeur par d�faut");
	...
	for(int i=0 ; i<_nbrParticules ; i++)	{	// Initialisation des particules
		...
		if(i < _nbrParticulesOnGround) {
			...
			// Cr�e les particules qui tombent
			randomPosition(flocon);
...
void MoteurNeige::refresh(CGame *game) {
	...
	// Quand le nombre de flocons au sol d�passe le seuil on refait tomber les premiers � �tre arriver au sol
