...
#include "FindFolder2.h"

// Pour rechercher c:/rep/fich*.map on aura par exemple :
//	CFindFolder( "c:/rep", "fich", ".map" );
//	ou
//	CFindFolder( "c:/rep", "fich, ".map" );
CFindFolder2::CFindFolder2(const char *filter, const char *optfilter1, const char *optfilter2) {
	if( !filter ) {
		cerr << endl << __FILE__ << ":" << __LINE__ << " CFindFolder2::CFindFolder2() filtre null" << endl;
...
int CFindFolder2::nbr() {	// Nombre de fichiers du répertoire correspondant aux filtres
	string name;
	int num = 0;
...
bool CFindFolder2::findNext(string &fichier)
{
#ifdef WIN32
...
int CFindFolder2::rmdir(const char *dir) {
#ifdef WIN32
	return _rmdir( dir );
#elif defined(__linux__)
	return ::rmdir( dir );
...
bool CFindFolder2::isFolder(const string& directory) {
...
bool CFindFolder2::chmod( char const *path, bool read, bool write )
{
#ifdef WIN32
	int opt = 0;
...
int CFindFolder2::mkdir( const char *dir )
{
#ifdef WIN32
	return _mkdir( dir );
#elif defined(__linux__)
