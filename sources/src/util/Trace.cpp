...
Trace::Trace() {
	...
	if(logConfig) {
		...
		// Lit les cl� et valeurs dans le fichier de configuration des log
...
void Trace::rotate() {
	...
	// Si un fichier de traces existe d�j�, alors renomme-le puis tente � nouveau de l'ouvrir
	if( std::ifstream( logFile.str().c_str()) ) {
		path logPath(TRACE_FOLDER);

		if(!exists(logPath)) {
			cerr << endl << "Le r�pertoire '" << TRACE_FOLDER << "' n'existe pas";
			return;
		}

		// Recherche un index de fichier de traces disponible apr�s les index d�j� utilis�s
		...
		for ( directory_iterator itr( logPath ); itr != end_itr; ++itr ) {
			if ( is_directory( *itr ) ) {
				...
				if( jkt::StringUtils::isBeginingWith(filename, PREFIX_FICHIER_TRACE) && jkt::StringUtils::isFinishingWith(filename, EXT_FICHIER_TRACE) ) {
					filename = string( filename.begin()+strlen(PREFIX_FICHIER_TRACE), filename.end()-strlen(EXT_FICHIER_TRACE) );	// Extraction du num�ro du fichier trace
		...
		numeroFichier++;	// Num�ro nouveau fichier = plus grand num�ro d'ancien fichier + 1
...
bool Trace::isLogLevelEnabled(TraceLevel level, const char *nomFichier) {
	bool youHaveToLog = (level >= _loggerLevel);	// Si on ne veut pas cette trace-l� alors sors
	string nomCourtFichier;

	// V�rifie si le fichier n'est pas exclu des log
	if(youHaveToLog) {
...
void Trace::print(TraceLevel level, TraceType type, int line, const char *nomFichier, const char *txt, va_list &vl) {
	...
	if(youHaveToLog) {
		bool prec = false;	// Indique que le caract�re pr�c�dent �tait un %
		void *THIS = 0;
		bool bTHIS = false;
		...
		// Message pr�c�d� �ventuellement par Begin ou End si c'est une TraceMethod
		switch( type ) {
		case TRACE_BEGIN:
			ligne << "Begin - ";
			break;
