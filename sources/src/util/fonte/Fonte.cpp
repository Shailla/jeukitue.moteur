...
void Fonte::load(const string& fonte, int height) {
	...
	for (int i = 32; i < MAXASCIICHAR; i++) {
		...
		if (rowWidth + g->bitmap.width + 1 >= MAXWIDTH) {	// Si on est au bout de la ligne on passe � la suivante
	...
	for (int i = 32; i < MAXASCIICHAR; i++) {
		...
		/* ************************************************ */
		/* Coordonn�es relatives � l'�cran d'affichage      */
		/* ************************************************ */

		lettres[i]._width = float(g->bitmap.width);
		lettres[i]._height = float(g->bitmap.rows);

		// Position de ce caract�re par rapport au curseur
...
void Fonte::getStringSize(const string& text, float scalar, float& stringWidth, float& stringHeight) const {
	...
	stringWidth = x + lettres[lettre]._width * scalar;	// On oublie pas la largeur de la derni�re lettre
	stringHeight = height;
}
